...
export async function writeSetupConfig(cwd: string, update: SetupConfigUpdate): Promise<string> {
...
function printSetupHelp(): void {
  console.log(
    [
      'azri setup — first-run setup wizard',
      '',
      'USAGE',
      '  azri setup',
      '',
...
export async function runSetup(args: string[]): Promise<number> {
  ...
  try {
    ...
    const configPathWritten = await writeSetupConfig(cwd, {
